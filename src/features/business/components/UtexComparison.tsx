      ...
      <article ref={ref} className="max-w-6xl">
        ...
        <ul className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {comparison.items[0].steps.map((step, idx) => (
            <div key={step.id} className="biz_item flex flex-col lg:flex-row items-center gap-6 rounded-lg bg-white p-6 shadow-md">
              <img src={`/img/product/compare/p${idx + 1}.png`} alt={step.content} className="h-64 w-64 rounded-lg object-cover shadow-sm sm:w-full sm:h-96 md:h-60 md:w-60" />
              <div>
      ...
      <article ref={ref} className="max-w-6xl pt-20">
        ...
        <ul className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {comparison.items[1].steps.map((step) => (
            <div key={step.id} className="biz_item bg-primary-50 border-primary flex flex-col lg:flex-row items-center gap-6 rounded-lg border p-6 shadow-md">
              <img src="/img/product/compare/p4.png" alt={step.content} className="h-64 w-64 rounded-lg object-cover shadow-sm sm:w-full sm:h-96 md:h-60 md:w-60" />
              <div>
