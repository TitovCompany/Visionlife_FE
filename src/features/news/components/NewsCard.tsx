...
const NewsCard: React.FC<NewsCardProps> = (props) => {
  ...
  return (
    <li className="w-full h-full rounded-xl">
      <Link to={`/company/news/${news.id}`} className="w-[350px] flex flex-col transition duration-300 hover:opacity-80 shadow-md overflow-hidden rounded-xl">
        <img
          src={isDefaultImg
          ...
          loading="lazy"
        />
        <div className="w-full h-40 pt-6 pb-10 px-6 flex flex-col flex-grow bg-[#f3f3f3] rounded-b-xl">
          <h3 className="w-full mb-2 text-lg font-semibold text-gray-800 break-words whitespace-normal">
            {news.title}
          </h3>
          {isDescription && (
